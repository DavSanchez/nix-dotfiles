...
in
{
  ...
  # Disable the upstream module so we can redefine options and add launchd support.
  disabledModules = [ "services/tldr-update.nix" ];
  ...
  config = lib.mkIf cfg.enable {
    ...
    assertions = [
      (lib.hm.darwin.assertInterval "tldr-update.period.*" cfg.period pkgs)
    ];
