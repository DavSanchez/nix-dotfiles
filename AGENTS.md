...
- Deploy the `deploy-rs` nodes in `flake.nix` with `deploy .#<host>`: `eter` (`eter.local`), `mora` (`mora.local`), `bruma` (`bruma.local`). A Pi flashed with `just sd-image <host>` already boots as `david` with the repo's keys, so `deploy` works normally from the first boot; a Pi flashed some other way (e.g. Hydra's generic `nixos.sd_image.aarch64-linux`) boots as `root`/`nixos` instead, so its first rollout needs `deploy --ssh-user root --hostname <ip> .#<host>`.
