...
with lib; let
  ...
  pam_reattach = pkgs.callPackage ./pam_reattach.nix {};
    ...
    ${
      ...
      then ''
        ...
        if ! grep '${option}' ${file} > /dev/null; then
          ${sed} -i '2i\
        auth       optional       ${pam_reattach}/lib/pam/pam_reattach.so # nix-darwin: ${option}
          ' ${file}
        fi
      ''
      else ''
...
in {
  options = {
    security.pam.enableSudoTouchIdAuthWithReattach = mkEnableOption ''
      ...
      When enabled, this option adds the following line to /etc/pam.d/sudo:
          ...
          auth       optional       ${pam_reattach}/lib/pam/pam_reattach.so
  ...
  config = {
    system.activationScripts.extraActivation.text = ''
      # PAM settings
      echo >&2 "setting up pam (with reattach)..."
